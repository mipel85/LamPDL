...
class FinancialRequestsMonitoringListController extends DefaultModuleController
{
    ...
    private function build_table($request)
    {
        ...
        if (in_array(PREFIX . 'financial_request_' . $request_fiscal_year, $request_archive_tables))
            $current_year = '_' . $request_fiscal_year;
        else
            $current_year = '';
        ...
        $total_amount = PersistenceContext::get_querier()->select('SELECT sum(amount_paid) AS total_expenses FROM ' . FinancialSetup::$financial_request_table . $current_year);
        while ($row = $total_amount->fetch())
        {
            $total_expenses = $row['total_expenses'];
        }

        $total_expenses = number_format((float)$total_expenses, 2, ',', '');
        $request_fiscal_year = $request->get_value('year', ' en cours');
        $table_model->set_caption($this->lang['financial.budgets.statement'] . ' - exercice ' . $request_fiscal_year . ' ... ' . $total_expenses . '€');
    ...
    private function build_select_year(HTTPRequestCustom $request)
    {
        ...
        $request_archive_tables = PersistenceContext::get_dbms_utils()->list_module_tables('financial_request_');
        if (in_array(PREFIX . 'financial_request_' . $request_fiscal_year, $request_archive_tables))
            $current_year = $request_fiscal_year;
        else
            $current_year = '';
